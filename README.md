...
`crawler.Crawl()` with delaying extracting:
...
`crawler.Crawl()` with processing a `robots.txt` file on extracting:
...
`crawler.Crawl()` with processing a `robots.txt` file on handling:
